...
echo "Installing nVidia drivers for ($cluster_name)"
kubectl create namespace gpu-resources
kubectl apply -f "${bootstrap_cluster_sh_script_path}/resources/bootstrap_cluster/nvidia-device-plugin-ds.yml"

# Tainting AKS GPU Nodes
echo "Tainting AKS GPU Nodes ($cluster_name)"
kubectl get nodes -o name | grep aks-gpupool | cut -d'/' -f 2 | while read -r name count; do
    echo "Tainting node: " $name
    kubectl taint node $name sku=gpu:NoSchedule
done
...
helm3 install nginx-ingress stable/nginx-ingress --set controller.service.annotations."service\.beta\.kubernetes\.io/azure-load-balancer-internal"='"true"' --namespace $nginx_ingress_namespace
